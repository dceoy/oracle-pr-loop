...
Treat exact final stderr `✖ read ETIMEDOUT` or stdout's last nonblank `ERROR:` line exactly equal to `ERROR: read ETIMEDOUT` as terminal for Oracle transport and never replay it. Instead, perform one read-only recovery check with `gh api --paginate` against that PR's review collection. Accept recovered publication only when exactly one persisted review has state `COMMENTED` and its top-level body contains the exact per-run correlation marker. This exact marker is positive proof of publication, not a heuristic. Do not use review counts, timestamps, reviewer identity, partial-marker matches, stdout content, or absence of the marker to infer publication or non-publication. If the GitHub read fails, zero reviews match, or more than one review matches, keep publication state **indeterminate** and fail closed.
