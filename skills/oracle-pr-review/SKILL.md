...
Treat exact final stderr `✖ read ETIMEDOUT` or stdout's last nonblank `ERROR:` line exactly equal to `ERROR: read ETIMEDOUT` as terminal for Oracle transport and never replay it. `--wait` and the heartbeat are preventive transport controls, not evidence that a timed-out accepted run is safe to replay. Instead, resolve the directory containing this loaded `SKILL.md` once as `skill_dir`, then invoke the bundled recovery script exactly once with the validated repository, PR number, and per-run review token. Claude Code may use `${CLAUDE_SKILL_DIR}` for `skill_dir`; other Agent Skills hosts should use the filesystem path from which they loaded this skill.
