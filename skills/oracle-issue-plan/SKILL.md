...
this rule to other `ETIMEDOUT` text, generic timeouts, disconnects, TLS errors, local-browser failures, or unrelated
transport failures.
...
each retry, sample Bash `$RANDOM` once and apply the existing 0.750 through 1.000 jitter multiplier:
...
If Oracle exits non-zero after the retry policy is exhausted or the response shows that the GitHub app was not invoked
or lacked repository access, report the failure. Otherwise return Oracle's implementation plan without rewriting it.
