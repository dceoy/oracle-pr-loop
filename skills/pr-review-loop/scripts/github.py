    ...
    *,
    ...
    if re.fullmatch(r"[0-9]+", value) is not None:
        if origin_repo is None:
            raise ReviewLoopError(EXIT_PRECONDITION, "input", numeric_message)
        ...
        if (
            ...
            raise ReviewLoopError(EXIT_PRECONDITION, "input", invalid_message)
        try:
            parsed = urllib.parse.urlsplit(value)
        except ValueError:
            raise ReviewLoopError(
                EXIT_PRECONDITION, "input", invalid_message
            ) from None
        target = re.fullmatch(
            rf"/([^/]+)/([^/]+)/{re.escape(route)}/([0-9]+)", parsed.path
        )
        if (
            parsed.scheme != "https"
...
def resolve_issue_target(value: str, origin_repo: str | None) -> tuple[str, int, str]:
    ...
    repository, number = _resolve_target(
        ...
        invalid_message="--issue must be a positive number or canonical GitHub issue URL",
        target_name="issue",
    )
...
def validate_path(path: str) -> str:
    ...
    if (
        ...
        raise ReviewLoopError(EXIT_PRECONDITION, "path", "invalid changed path")
    ...
    if (
        ...
        raise ReviewLoopError(
            EXIT_PRECONDITION, "path", f"unsafe changed path: {path}"
        )
    return path
...
class DiffFileAnalysis:
    """Structural facts derived from one canonical frozen-diff file section."""
...
class FrozenDiffAnalysis:
    """One fail-closed structural analysis of the exact frozen unified diff."""
...
def _consume_hunk_body(line: str, state: _SectionState) -> bool:
    ...
    if path is not None:
        if marker == "-" and state.old_line > 0:
            state.anchors.add((path, "LEFT", state.old_line))
        elif marker in {" ", "+"} and state.new_line > 0:
            state.anchors.add((path, "RIGHT", state.new_line))
...
def analyze_frozen_diff(
    patch: bytes, allowed_paths: frozenset[str]
) -> FrozenDiffAnalysis:
    """Derive all unified-diff structural facts in one primary pass.

    The pass owns path interpretation, exact LEFT/RIGHT line semantics,
    `--full-index` blob ids, base-path mapping, and per-file section sizes.
    Malformed or unverifiable sections contribute no anchors. Aggregate and
    per-file GitHub truncation boundaries are conservative: equality is unsafe.
    """
    try:
        text = patch.decode("utf-8", "strict")
    except UnicodeDecodeError as exc:
        raise ReviewLoopError(
            EXIT_PRECONDITION, "bundle", "patch is not UTF-8"
        ) from exc
    raw_lines = patch.split(b"\n")
    text_lines = text.split("\n")
    if len(raw_lines) != len(text_lines):
        return FrozenDiffAnalysis(frozenset(), {})
    ...
    for index, (raw_line, line) in enumerate(zip(raw_lines, text_lines, strict=True)):
        raw_size = len(raw_line) + int(index < len(raw_lines) - 1)
        physical = int(bool(raw_line) or index < len(raw_lines) - 1)
        if line.startswith("diff --git "):
            _finalize_diff_section(state, allowed_paths, files, anchors, invalid_paths)
            state = _SectionState(byte_size=raw_size, line_count=physical)
            continue
        if state is None:
            continue
        state.byte_size += raw_size
        state.line_count += physical
        ...
        if line.startswith("index "):
            ...
            state.head_path = (
                _header_path(line.removeprefix("+++ "), "b/") or state.old_path
            )
        elif (match := HUNK_RE.match(line)) is not None:
...
def parse_json_object(text: str, *, category: str) -> JsonObject:
    ...
    try:
        ...
        raise ReviewLoopError(
            EXIT_GITHUB, category, "GitHub returned malformed JSON"
        ) from exc
    if not isinstance(value, dict) or not all(isinstance(key, str) for key in value):
        raise ReviewLoopError(
            EXIT_GITHUB, category, "GitHub returned a non-object JSON response"
        )
...
def require_object(value: JsonValue | None, *, field: str) -> JsonObject:
    ...
    if not isinstance(value, dict):
        raise ReviewLoopError(
            EXIT_GITHUB, "github_schema", f"GitHub field {field} must be an object"
        )
    return value
...
def require_string(
    ...
    if not isinstance(value, str) or (not allow_empty and not value):
        raise ReviewLoopError(
            EXIT_GITHUB, "github_schema", f"GitHub field {field} must be a string"
        )
    return value
...
def require_integer(value: JsonValue | None, *, field: str) -> int:
    ...
    if isinstance(value, bool) or not isinstance(value, int):
        raise ReviewLoopError(
            EXIT_GITHUB, "github_schema", f"GitHub field {field} must be an integer"
        )
    return value
...
def require_boolean(value: JsonValue | None, *, field: str) -> bool:
    ...
    if not isinstance(value, bool):
        raise ReviewLoopError(
            EXIT_GITHUB, "github_schema", f"GitHub field {field} must be a boolean"
        )
    return value
...
class _ImmutableGitMixin:
        ...
        *,
        ...
        try:
            result = self.runner.run(
                ["git", *args], cwd=self.repo_dir, env=env, max_output=max_output
            )
    ...
    def _initialize_repository(self, *, hardened: bool, require_push_url: bool) -> str:
        ...
        if not root:
            raise ReviewLoopError(
                EXIT_PRECONDITION, "repository", "Git returned an empty repository root"
            )
    ...
    def ensure_commit_object(self, sha: str) -> None:
        object_type = self.git_bytes(
            ["cat-file", "-t", sha], max_output=1024
        ).decode("utf-8", "strict")
    ...
    def tracked_paths_at(self, sha: str) -> tuple[str, ...]:
        output = self.git_bytes(
            ["ls-tree", "-r", "-z", "--name-only", sha], max_output=4 * 1024 * 1024
        )
        paths: list[str] = []
        ...
        if len(paths) != len(set(paths)):
            raise ReviewLoopError(
                EXIT_PRECONDITION, "path", "Git tree returned duplicate tracked paths"
            )
        return tuple(sorted(paths))

    def blob_bytes_at(
        self, sha: str, path: str, *, max_output: int
    ) -> bytes | None:
        ...
        if len(records) != 1 or b"\t" not in records[0]:
            raise ReviewLoopError(
                EXIT_PRECONDITION, "git", "Git tree returned an ambiguous changed-file entry"
            )
        metadata, raw_path = records[0].split(b"\t", 1)
        fields = metadata.split()
        if len(fields) != LS_TREE_FIELD_COUNT:
            raise ReviewLoopError(
                EXIT_PRECONDITION, "git", "Git tree returned malformed changed-file metadata"
            )
        ...
        if listed_path != path or not SHA_RE.fullmatch(object_sha):
            raise ReviewLoopError(
                EXIT_PRECONDITION, "git", "Git tree changed-file identity mismatched"
            )
        ...
        if size > max_output:
            return None
        data = self.git_bytes(["cat-file", "blob", object_sha], max_output=max_output)
        if len(data) != size:
            raise ReviewLoopError(
                EXIT_PRECONDITION,
                "git",
    ...
    def _check_attr_diff_isolated(
        ...
        common_dir = Path(
            self.git_bytes(["rev-parse", "--git-common-dir"], max_output=4096)
            .decode("utf-8", "strict")
            .strip()
        )
...
class GitHubClient(_ImmutableGitMixin):
    ...
    def initialize(self, pr_value: str) -> None:
        origin_repo = self._initialize_repository(hardened=True, require_push_url=False)
        ...
        if not self.authenticated_login:
            raise ReviewLoopError(
                EXIT_GITHUB, "identity", "authenticated GitHub login was empty"
            )

    def initialize_for_submit(self, pr_value: str) -> None:
        origin_repo = self._initialize_repository(hardened=False, require_push_url=True)
        self._set_target(pr_value, origin_repo)
    ...
    def snapshot(self, *, require_open: bool = True) -> PullRequest:
        ...
        if not isinstance(files_value, list):
            raise ReviewLoopError(
                EXIT_GITHUB, "github_schema", "GitHub field files must be an array"
            )
        paths: list[str] = []
        for item in files_value:
            ...
            paths.append(validate_path(require_string(item.get("path"), field="files.path")))
        advertised_count = require_integer(data.get("changedFiles"), field="changedFiles")
        ...
        if len(paths) != len(set(paths)):
            raise ReviewLoopError(EXIT_PRECONDITION, "path", "duplicate changed paths")
        author = require_object(data.get("author"), field="author")
        author_login = require_string(author.get("login"), field="author.login")
        if not author_login:
            raise ReviewLoopError(
                EXIT_PRECONDITION, "identity", "pull request author was empty"
            )
        return PullRequest(
    ...
    def identity_snapshot(self, *, require_open: bool = True) -> PullRequestIdentity:
        return self._parse_identity(
            self._pull_request_data(PR_IDENTITY_FIELDS), require_open=require_open
        )

    def _pull_request_data(self, fields: str) -> JsonObject:
        return parse_json_object(
            self._text(
                ["pr", "view", self.url, "--json", fields], max_output=8 * 1024 * 1024
            ),
            category="github_schema",
        )

    def _parse_identity(
        self, data: JsonObject, *, require_open: bool
    ) -> PullRequestIdentity:
        head_repository = require_object(data.get("headRepository"), field="headRepository")
        head_owner = require_object(
            data.get("headRepositoryOwner"), field="headRepositoryOwner"
        )
        if "nameWithOwner" in head_repository:
            head_repo = require_string(
                head_repository.get("nameWithOwner"), field="headRepository.nameWithOwner"
            )
        else:
            owner = require_string(
                head_owner.get("login"), field="headRepositoryOwner.login"
            )
            name = require_string(head_repository.get("name"), field="headRepository.name")
    ...
    def _validate_snapshot_identity(
        self, pull_request: PullRequestIdentity, *, require_open: bool
    ) -> None:
        if (
            ...
            raise ReviewLoopError(
                EXIT_PRECONDITION, "identity", "ambiguous pull request identity"
            )
        if require_open and (pull_request.state != "OPEN" or pull_request.is_draft):
            raise ReviewLoopError(
                EXIT_PRECONDITION, "state", "pull request must be open and non-draft"
            )
        if pull_request.head_repository.lower() != self.repository.lower():
            raise ReviewLoopError(
                EXIT_PRECONDITION, "repository", "fork pull requests are not supported"
            )
        if not SHA_RE.fullmatch(pull_request.base_sha) or not SHA_RE.fullmatch(
            pull_request.head_sha
        ):
            raise ReviewLoopError(
                EXIT_PRECONDITION, "sha", "invalid base or head SHA"
            )
    ...
    def review_event(self, pull_request: PullRequest, verdict: str) -> str:
        if verdict not in {"APPROVE", "REQUEST_CHANGES"}:
            raise ReviewLoopError(
                EXIT_PRECONDITION, "verdict", "invalid review verdict"
            )
    ...
    def changed_file_bytes(
        self, pull_request: PullRequest, path: str, *, max_output: int
    ) -> bytes | None:
        return self.blob_bytes_at(pull_request.head_sha, path, max_output=max_output)
    ...
    def diff_anchors(
        ...
        """Return exact frozen-diff anchors after binary and attribute validation."""
        analysis = analyze_frozen_diff(
            self.patch(pull_request, max_output=MAX_ANCHOR_PATCH_BYTES),
        ...
        for head_path in candidate_paths:
            ...
            if file_analysis is None:
                continue
            head_paths_by_base_path.setdefault(file_analysis.base_path, []).append(head_path)
        if len({p for values in head_paths_by_base_path.values() for p in values}) != len(
            candidate_paths
        ):
            return frozenset()
        ...
        data = parse_json_object(
            self._text(
                [
                    ...
                    "--hostname",
                    "github.com",
                    f"repos/{pull_request.repository}/pulls/{pull_request.number}/reviews",
                    "--method",
                    "POST",
                    "--input",
                    "-",
    ...
    def dismiss(self, pull_request: PullRequest, review_id: int) -> None:
        ...
        self._text(
            [
                ...
                "--hostname",
                "github.com",
                f"repos/{pull_request.repository}/pulls/{pull_request.number}/reviews/{review_id}/dismissals",
                "--method",
                "PUT",
                "--input",
                "-",
    ...
    def verify_posted(
        self, pull_request: PullRequest, review_id: int, body: str
    ) -> JsonObject:
        data = parse_json_object(
            self._text([
                "api",
                "--hostname",
                "github.com",
                f"repos/{pull_request.repository}/pulls/{pull_request.number}/reviews/{review_id}",
            ]),
            category="github_schema",
        )
        if (
            ...
            raise ReviewLoopError(
                EXIT_GITHUB, "github", "posted review revalidation failed"
            )
        return data
...
class IssueClient(_ImmutableGitMixin):
    ...
    def snapshot(self) -> IssueSnapshot:
        ...
        repository_data = require_object(
            response_data.get("repository"), field="data.repository"
        )
        data = require_object(
        ...
        if len(body.encode("utf-8")) > MAX_ISSUE_BODY_BYTES:
            raise ReviewLoopError(
                EXIT_PRECONDITION, "bundle", "issue body exceeds bound"
            )
        issue = IssueSnapshot(
            repository=self.repository,
            number=require_integer(data.get("number"), field="number"),
    ...
    def _validate_snapshot_identity(self, issue: IssueSnapshot) -> None:
        if issue.number != self.number or issue.url.rstrip("/").lower() != self.url.lower():
            raise ReviewLoopError(
                EXIT_PRECONDITION, "identity", "ambiguous issue identity"
            )
        if issue.state != "OPEN":
    ...
    def branch_sha(self, branch: str) -> str:
        ...
        if not SHA_RE.fullmatch(sha):
            raise ReviewLoopError(EXIT_PRECONDITION, "sha", "invalid base SHA")
        return sha
