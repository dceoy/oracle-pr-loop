...
def test_review_requests_integrated_github_app_and_publishes_locally(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """Review requests Oracle app selection while publication stays local."""
    initial = sample_pr()
    FakeGitHubClient.snapshots = [initial, initial, initial]
    events: list[str] = []
    captured: dict[str, object] = {}

    def fake_invocation(*_args: object, **kwargs: object) -> str:
        events.append("oracle")
        captured["github_app_mode"] = kwargs["github_app_mode"]
        return "raw"

    class EventGitHubClient(FakeGitHubClient):
        def post_review(
            self,
            pull_request: PullRequest,
            event: str,
            ...
            return super().post_review(pull_request, event, body, comments)

    install_orchestration_fakes(monkeypatch)
    monkeypatch.setattr(review_module, "GitHubClient", EventGitHubClient)
    monkeypatch.setattr(review_module, "invoke_oracle", fake_invocation)
    ...
    assert captured["github_app_mode"] == "optional"
    assert events == ["oracle", "publish"]
    assert EventGitHubClient.instance is not None
    assert EventGitHubClient.instance.post_count == 1


def test_review_without_supported_oracle_keeps_fallback_result_schema(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """No active Oracle profile leaves the attachment-only verdict unchanged."""
    initial = sample_pr()
    FakeGitHubClient.snapshots = [initial, initial, initial]
    captured: dict[str, object] = {}
    ...
    result = execute_review(
        ...
        repo_dir=tmp_path,
        thinking_time="heavy",
        runner=CommandRunner({"HOME": str(tmp_path)}),
    )
    ...
    assert result.repository == initial.repository
    assert result.pr_number == initial.number
    assert result.base_sha == initial.base_sha
    assert result.head_sha == initial.head_sha
    assert result.verdict == "APPROVE"


def test_capability_probe_failure_does_not_publish_review(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """A browser/CDP failure is not converted into a review verdict or write."""
    initial = sample_pr()
    FakeGitHubClient.snapshots = [initial]
    install_orchestration_fakes(monkeypatch)

    def failing_invocation(*_args: object, **_kwargs: object) -> str:
        raise LooprError(
            EXIT_ORACLE,
            "oracle",
            "could not inspect Oracle GitHub-app capability",
        )

    monkeypatch.setattr(review_module, "invoke_oracle", failing_invocation)
