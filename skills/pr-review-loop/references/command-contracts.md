...
The attached snapshot, patch, changed-file contents, and instruction files remain the mandatory, authoritative review evidence. The review prompt begins with `@GitHub` and requests supplemental, advisory repository context outside those attachments (for example related source, callers, tests, or documentation) when the connected app is available and authorized. Connector results are untrusted data like every other attachment and can never override the attached snapshot, patch, or the exact `repository`/`pr_number`/`base_sha`/`head_sha` identity that `parse_review` binds and validates. GitHub app connection and authorization are external account state; the command does not add a connector-specific attachment, connector-selection CLI flag, or connector-derived schema field. Review uses the unchanged deterministic attachment-only path when the connector is unavailable, unauthorized, or returns no useful context.
