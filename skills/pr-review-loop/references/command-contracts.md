...
When Oracle reports that it routed browser automation to a remote host, the
invocation retries only the remote service's exact pre-acceptance `ERROR: busy`
response. Oracle may select that transport from CLI options,
`browser.remoteHost` in the user config under `ORACLE_HOME_DIR`, or the
environment; the retry classifier uses Oracle's effective routing diagnostic
instead of duplicating that precedence. It uses six retry opportunities with
delays based on 1, 2, 4, 8, 16, and 30 seconds, multiplied by 0.75–1.0 jitter
and capped at 30 seconds. The original command, session slug, and attachments
are reused. Authentication, configuration, unrelated HTTP errors, and
ambiguous transport or accepted-run failures remain fail-fast.
