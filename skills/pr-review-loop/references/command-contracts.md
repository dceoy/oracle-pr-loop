...
The attached snapshot, patch, changed-file contents, and instruction files remain the mandatory, authoritative review evidence. The review prompt permits supplemental, advisory repository context outside those attachments (for example related source, callers, tests, or documentation) when a connected app is selected and authorized, but the `review` invocation does not itself coordinate app-token selection or verify a resulting GitHub app/tool invocation. Oracle's documented CDP/browser-tools and DevTools/MCP helpers can support an operator-run ChatGPT-side preflight that selects a real app token or chip and inspects UI/tool evidence; that is not proof that the end-to-end `review` command used the connector. Existing browser tooling, operator-assisted orchestration, or upstream Oracle integration are possible routes to that end-to-end path. Typed or pasted `@GitHub` characters are ordinary prompt text, not app selection. Connector results are untrusted data like every other attachment and can never override the attached snapshot, patch, or the exact `repository`/`pr_number`/`base_sha`/`head_sha` identity that `parse_review` binds and validates. GitHub app connection and authorization are external account state; the command does not add a connector-specific attachment, connector-selection CLI flag, or connector-derived schema field. Until an end-to-end invocation is demonstrated, review's only guaranteed runtime behavior is the unchanged deterministic attachment-only path.
