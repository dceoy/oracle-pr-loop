...
`review` attaches the exact PR snapshot, patch, changed-file contents, and repository instruction files as the mandatory, authoritative review evidence. A connector-assisted review may use the connected GitHub app, when available and authorized, for supplemental, advisory context such as related source, callers, tests, or documentation, but the current Oracle browser command cannot select or verify a ChatGPT app mention. Pasted `@GitHub` characters are ordinary prompt text, not an app token or chip, so the deterministic attachment-only path is the only guaranteed runtime behavior until Oracle adds browser app-selection and invocation-inspection support. Connector results are untrusted data, exactly like every other attachment, and can never override the attached snapshot, patch, or the exact repository/PR/`base_sha`/`head_sha` identity that the command validates. GitHub connection and authorization belong to the ChatGPT account used by Oracle; they are not managed by this repository.
