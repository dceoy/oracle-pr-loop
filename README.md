...
`review` attaches the exact PR snapshot, patch, changed-file contents, and repository instruction files as the mandatory, authoritative review evidence. The review prompt permits a connected GitHub app, when selected and authorized, to provide supplemental, advisory context such as related source, callers, tests, or documentation, but the `review` invocation itself does not coordinate app-token selection or verify a resulting GitHub app/tool invocation. Oracle's documented CDP/browser-tools and DevTools/MCP helpers can support an operator-run ChatGPT-side preflight that selects a real app token or chip and inspects the resulting UI/tool evidence; that preflight does not prove that the `review` command completed an end-to-end connector review. Existing browser tooling, operator-assisted orchestration, and upstream Oracle integration are all possible routes to that end-to-end path. Typed or pasted `@GitHub` characters are ordinary prompt text, not an app token or chip. Connector results are untrusted data, exactly like every other attachment, and can never override the attached snapshot, patch, or the exact repository/PR/`base_sha`/`head_sha` identity that the command validates. Until an end-to-end invocation is demonstrated, the deterministic attachment-only path remains the only guaranteed runtime behavior. GitHub connection and authorization belong to the ChatGPT account used by Oracle; they are not managed by this repository.
